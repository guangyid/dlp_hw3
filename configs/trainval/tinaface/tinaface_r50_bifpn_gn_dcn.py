...
data = dict(
    ...
    train=dict(
        ...
        pipeline=[
            dict(typename='LoadImageFromFile', to_float32=True,file_client_args = dict(
                backend = 'lmdb',
                db_path = data_root + 'WIDER_train/train'
            )),
    ...
    val=dict(
        ...
        pipeline=[
            dict(typename='LoadImageFromFile', to_float32=True,file_client_args = dict(
                backend = 'lmdb',
                db_path = data_root + 'WIDER_val/val'
            )),
            dict(
...
hooks = [
    ...
    dict(typename='SnapshotHook', interval=5,max_keep_ckpts=10),
...
# 6. checkpoint
weights = dict(
    filepath='/model/guangyid/resnet50/resnet50-19c8e357.pth',
    prefix='backbone')
